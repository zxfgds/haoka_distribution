...
final class ChangeVirtualPhoneNumbersTable extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    {
        $this->table('product_virtual_numbers')->drop()->save();
        $table = $this->table('product_fancy_numbers');
        $table->addColumn('number', 'string', ['limit' => 11, 'comment' => '手机号码'])
              ...
              ->addColumn('is_virtual', 'boolean', ['comment' => '是否虚拟号', 'default' => TRUE])
        ...
        /**
         * 根据上面数组, 生成phinx 的表迁移文件, 表名叫做`product_fancy_numbers`,字段中要带有注释, 你根据数组的注释判断字段的长度和类型, 并为字段添加注释
         */
